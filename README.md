...
> [!IMPORTANT]
> Don't forget to adjust/replace hardcoded values in [`nix/variables.nix`](nix/variables.nix) and [`ansible/group_vars/all.yaml`](ansible/group_vars/all.yaml) before running the configuration for the first time.
