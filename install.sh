...
scaffold_machine() {
  ...
  {
    printf '{ user, ... }:\n\n'
    printf 'let\n  stateVersion = throw "copy system.stateVersion from the generated config below";\nin\n{\n'
    printf '  imports = [\n    ./hardware-configuration.nix\n    ../../profiles/workstation.nix\n  ];\n\n'
    printf '  # boot.loader.* goes here, plus boot.initrd.luks.* if this machine is encrypted.\n\n'
    printf '  networking.hostName = "%s";\n\n' "$host"
    printf '  system.stateVersion = stateVersion;\n'
    printf '  home-manager.users.${user}.home.stateVersion = stateVersion;\n}\n\n'
    printf '%s\n' "$rule"
    printf '# Below is the autogenerated /etc/nixos/configuration.nix. Most of its options\n'
    printf '# are already covered by base/common.nix or the profiles and should be dropped.\n'
    printf '# Keep what this machine actually needs, then remove the rest.\n'
    printf '%s\n' "$rule"
    sed -e 's/^/# /' -e 's/[[:space:]]*$//' /etc/nixos/configuration.nix
  } >"$machine/default.nix"
}
...
bootstrap_nixos() {
  ...
  say "Before the installation proceeds, finish nix/machines/$host/default.nix: set stateVersion, lift the bootloader lines out of the commented block, then delete the rest (see existing machines for reference)."
